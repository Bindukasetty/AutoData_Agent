...
class DatabaseUtil:
    ...
    def execute_sql(self, query):
        try:
            ...
            connection.commit()
            # here we are converting to string because the loops returns list
            return str(result)
        except Exception as e:
            print(f"Error executing query: {e}")
            return None
        # final block will always execute even it returns result or exception. So, we can close the cursor and connection in the finally block.
        finally:
            if cursor:
                cursor.close()
            if connection:
...
# database calling code for testing the schema_details function

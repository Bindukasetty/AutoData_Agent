...
def generate_sql(state: AgentSchema) -> AgentSchema:
    ...
    state.generated_sql_query = generated_sql_query #generated SQL query is stored in the state

    return state

# AI as a judge to determine if the generated SQL query is safe to execute or not
# Is safe Node ? to check if the generated SQL query is safe to execute or not
...
# we create Nodes for each function
...
def is_safe_sql_edge(state: AgentSchema) -> str:
    ...
    if is_safe.lower() == "yes":
        return "execute_sql"  # we should return the node where it should go to after the evaluation of the condition. I want to go to the execute_sql node if the SQL query is safe to execute, else return cancel sql
...
# we cant update the state in parallel. we can enhance the map like above
...
# Compile the Graph (compile nodes,edges)
...
if __name__ == "__main__":
    ...
    # img = Image(sql_analyst.get_graph().draw_mermaid_png(draw_method=MermaidDrawMethod.PYPPETEER)) # mermaid is a diagram
    img = Image(sql_analyst.get_graph().draw_png())

...
def router_node(State: DataAgentSchema):
    ...
    route_response_dict = llm_router.invoke(messages).model_dump() # this will return a dictionary with the structured output
    route_response = route_response_dict['answer']
    State.route_response = route_response

# This function will route the user question to either the SQL agent or the ETL agent based on the response from the router node
